...
public class LambdaExample {
    public static void main(final String[] args) {
        ...
        Predicate<User> usersAgeMoreThan5 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age > 5;
            }
        };
        Comparator<User> sortUsersByName = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                return o1.name.compareTo(o2.name) ;
            }
        };
        repository.select(usersAgeMoreThan5, sortUsersByName);

        banner("Listing users with age > 5 sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(
                user -> user.age > 5 ,
                (o1,o2) -> o1.name.compareTo(o2.name)> 0 ? 1 : -1
        );

        banner("Listing users with age < 10 sorted by age");
        // TODO With functional interfaces declared
        Predicate<User> usersAgeLessThan10 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.age < 10 ;
            }
        };
        Comparator<User> sortUsersByAge = new Comparator<User>() {
            @Override
            public int compare(User o1, User o2) {
                if(o1.age > o2.age)
                    return 1;
        ...
        repository.select(usersAgeLessThan10, sortUsersByAge);
        ...
        repository.select(activeUserPredicate, sortUsersByName);
        ...
        // TODO With functional interfaces declared
        Predicate<User> ActiveusersWithAgeMoreThan8 = new Predicate<User>() {
            @Override
            public boolean test(User user) {
                return user.active && user.age > 8;
            }
        };
        repository.select(ActiveusersWithAgeMoreThan8, sortUsersByName);

        banner("Listing active users with age > 8 sorted by name - lambda");
        // TODO With functional interfaces used directly
        repository.select(
                user -> user.active && user.age > 8,
                (o1, o2) -> (o1.name.compareTo(o2.name) > 0) ? 1 : -1
        );
...
class UsersRepository {
    ...
    //predicate return bool, take a single value
